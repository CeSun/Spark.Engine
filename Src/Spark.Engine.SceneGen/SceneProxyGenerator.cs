...
public sealed class SceneProxyGenerator : IIncrementalGenerator
{
    ...
    private const string SceneResourceInterfaceName = "Spark.Engine.Resources.ISceneResource";
    ...
    private static string EmitComponentPartial(ComponentModel model)
    {
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            _registeredScene = scene;   // 缓存注册场景，EndPlay 用它反注册，跨世界不残留幽灵代理（中6）");
