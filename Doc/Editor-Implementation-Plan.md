...
当前状态：SceneDocument 和自定义二进制 `.scene` 保存/读取基础已落地；编辑器预览会初始化渲染代理但冻结 gameplay Tick，Viewport 已支持 CPU 包围球拾取和可撤销局部变换，
...
2. ⏳ Transform Gizmo：平移、旋转、缩放（命令层已就绪，轴绘制和拖拽约束待实现）。
3. 网格、吸附、世界/局部坐标。
4. 聚焦选中对象、相机控制和视图书签。
