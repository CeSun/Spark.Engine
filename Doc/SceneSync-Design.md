...
`BlinnPhongRenderer`（镜像 + GPU 资源），让所有场景对象（网格/光源/未来的骨骼/粒子…）共用一套身份与
...
`BlinnPhongRenderer` ≈ `FSceneRenderer` 输入侧。骨架同构，差异是早期简化（单一双缓冲、无命令流、无 RHI 线程）。
...
## 7. 渲染侧：`BlinnPhongRenderer`

渲染线程的镜像。持久部分（GPU 资源、每实例状态）跨帧保留，快照部分每帧覆盖：

```csharp
public sealed class BlinnPhongRenderer
{
...
| `BlinnPhongRenderer._proxyStates` | 渲染线程 | 仅渲染线程读写 |
...
+ 在 `BlinnPhongRenderer` 加消费分支**；proxy/payload/快照登记点/生命周期全部由生成器产出，同步机制、身份
协议、线程契约、剔除循环零改动。
...
| ID | 决策 | 备选 | 理由 |
|---|---|---|---|
| ADR-8 | 所有场景对象共用 `SceneProxy → SceneSnapshot → BlinnPhongRenderer` 单通道 | 每类独立一套列表/队列/注册表 | 一条线程契约、一套身份与生命周期协议，新增类别不复制机制 |
