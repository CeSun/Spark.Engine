...
- `UIPropertyGrid` 的 Vector2/Vector3/Vector4 输入统一使用 `<x, y, ...>` 格式。
- `Quaternion` 在编辑器中显示为三轴欧拉角（`Pitch, Yaw, Roll`，单位：度）。
- 三轴欧拉角提交时转换为 `Quaternion.CreateFromYawPitchRoll`；仍兼容四分量四元数输入。
...
- [x] 输入 `<1, 2, 3>` 后，Vector3 属性得到对应三个分量。
