...
> **验收状态：⚠ 未验收**（仅编译通过，未由用户运行 Demo 逐场景确认）。
...
**尚未做**：
- 用户未在本机运行 `dotnet run --project Demo\Demo.Desktop` 逐场景目视确认。
- 4 个验收 Overlay 的交互预期（计数器增减、异常标签文案、字形完整）未经人眼核对。

待用户运行后逐项确认；若发现回归，回退或补丁再修。
